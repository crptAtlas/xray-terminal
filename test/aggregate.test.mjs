...
test("grade waits for the holders record and then respects it", async () => {
  ...
  // once the record is known it is the grade: underwater is red
  assert.equal(gradeOf({ ...base, avgProfilePnl: -40, avgWinrate: 20 }, holders, false), "shattered");
  assert.equal(gradeOf({ ...base, avgProfilePnl: -1 }, holders, false), "shattered");
  // 0 to +25 yellow: in profit, but not by much
  assert.equal(gradeOf({ ...base, avgProfilePnl: 0 }, holders, false), "cracked");
  assert.equal(gradeOf({ ...base, avgProfilePnl: 7 }, holders, false), "cracked");
  assert.equal(gradeOf({ ...base, avgProfilePnl: 24 }, holders, false), "cracked");
  // clearly ahead is green
  assert.equal(gradeOf({ ...base, avgProfilePnl: 25 }, holders, false), "healthy");
  assert.equal(gradeOf({ ...base, avgProfilePnl: 31 }, holders, false), "healthy");
